...
export default function ConfiguracionesPage() {
    ...
    const loadData = async () => {
        try {
            setLoading(true);
            if (activeTab === "roles")
                setRoles(await apiClient.roles.list());
            if (activeTab === "fases")
                setFases(await apiClient.phases.list());
            if (activeTab === "gestion")
                setGestiones((await apiClient.gestiones.list()).data);
    ...
    const startEdit = (type: EditableType, item: any) => {
        ...
        if (type === "gestion")
            setFormData({ gestion: item.gestion, typeGestion: item.typeGestion, isActive: item.isActive });
        if (type === "linea")
            setFormData({
                name: item.name,
    ...
    const handleSave = async () => {
        ...
        try {
            if (editing.type === "fase") {
                ...
                if (editing.id)
                    await apiClient.phases.update(editing.id, formData.name);
                else
                    await apiClient.phases.create(formData.name);
            }
            if (editing.type === "gestion") {
                if (!formData.gestion || !formData.typeGestion)
                    throw new Error("Año y tipo son obligatorios");
                const payload = {
                ...
                if (editing.id)
                    await apiClient.gestiones.update(editing.id, payload);
                else
                    await apiClient.gestiones.create(payload);
            }
            if (editing.type === "linea") {
                if (!formData.name || !formData.idAreaInvestigacion)
                ...
                if (editing.id)
                    await apiClient.researchLines.update(editing.id, payload);
                else
                    await apiClient.researchLines.create(payload);
            }
            if (editing.type === "area") {
                if (!formData.name)
                    throw new Error("El nombre es obligatorio");
                if (editing.id)
                    await apiClient.researchAreas.update(editing.id, formData.name);
                else
                    await apiClient.researchAreas.create(formData.name);
            }
            if (editing.type === "semestre") {
                ...
                if (editing.id)
                    await apiClient.semesters.update(editing.id, payload);
                else
                    await apiClient.semesters.create(payload);
            }
            if (editing.type === "modalidad") {
                ...
                if (editing.id)
                    await apiClient.modalidades.update(editing.id, payload);
                else
                    await apiClient.modalidades.create(payload);
            }
            if (editing.type === "tipoDoc") {
                ...
                if (editing.id)
                    await apiClient.typeDocs.update(editing.id, payload);
                else
                    await apiClient.typeDocs.create(payload);
            }
    ...
    const renderCardActions = (item: any, type: EditableType) => {
        ...
        {isEditing ? (<>
            ...
            {type !== "semestre" && type !== "fase" && type !== "tipoDoc" && (<Button variant="ghost" size="sm" className="text-red-600" onClick={async () => {
                        if (!confirm("¿Eliminar este registro?"))
                            return;
                        try {
                            if (type === "gestion")
                                await apiClient.gestiones.delete(item.id);
                            if (type === "linea")
                                await apiClient.researchLines.delete(item.id);
                            if (type === "area")
                                await apiClient.researchAreas.delete(item.id);
                            if (type === "modalidad")
                                await apiClient.modalidades.delete(item.id);
    ...
    const renderInlineForm = (type: EditableType, item?: any) => {
        ...
        if (type === "gestion") {
          ...
          {item && (<div className="flex items-center gap-2 col-span-2">
        ...
        if (type === "linea") {
          ...
          <div className="grid gap-2">
            ...
            {areaOptions.length > 0 ? (<Select value={formData.idAreaInvestigacion?.toString()} onValueChange={(v) => setFormData({ ...formData, idAreaInvestigacion: Number(v) })}>
                <SelectTrigger>
                  <SelectValue placeholder="Selecciona un área"/>
                </SelectTrigger>
                <SelectContent>
                  {areaOptions.map((a) => (<SelectItem key={a.id} value={a.id.toString()}>
                      {a.name}
                    </SelectItem>))}
                </SelectContent>
              </Select>) : (<Input type="number" value={formData.idAreaInvestigacion || ""} onChange={(e) => setFormData({ ...formData, idAreaInvestigacion: Number(e.target.value) })} placeholder="ID de área (no listado por API)"/>)}
          </div>
        </div>);
        }
        if (type === "modalidad") {
    ...
    const renderCreateCard = (type: EditableType, title: string, description: string) => {
        ...
        return (<div className="border rounded-xl p-4 bg-muted/40 flex flex-col gap-3">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm font-semibold">{title}</p>
            <p className="text-xs text-muted-foreground">{description}</p>
          </div>
          {isCreating ? (<div className="flex gap-2">
              <Button size="sm" className="gap-1" onClick={handleSave}>
                <Save className="w-4 h-4"/> Guardar
              </Button>
              <Button variant="outline" size="sm" onClick={cancelEdit}>
            ...
            </div>) : (<Button size="sm" className="gap-1" onClick={() => startCreate(type)}>
    ...
    return (<div className="p-4 md:p-6 space-y-6 w-full">
      ...
      <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as any)} className="w-full">
        ...
        <TabsContent value="roles" className="pt-4 space-y-3">
          {loading ? (<CenteredLoader label="Cargando roles..." className="border rounded-xl bg-card"/>) : (<div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
              {roles.map((role) => (<div key={role.id} className="border rounded-xl p-4 bg-card shadow-sm flex flex-col gap-3 border-l-4 border-l-primary">
        ...
        <TabsContent value="fases" className="pt-4 space-y-3">
          ...
          {loading ? (<CenteredLoader label="Cargando fases..." className="border rounded-xl bg-card"/>) : (<div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
              {fases.map((f) => (<div key={f.id} className="border rounded-xl p-4 bg-card shadow-sm border-l-4 border-l-primary">
        ...
        <TabsContent value="gestion" className="pt-4 space-y-3">
          ...
          {loading ? (<CenteredLoader label="Cargando gestiones..." className="border rounded-xl bg-card"/>) : (<div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
              {gestiones.map((g) => (<div key={g.id} className="border rounded-xl p-4 bg-card shadow-sm border-l-4 border-l-primary">
                  <div className="flex items-start justify-between gap-2">
        ...
        <TabsContent value="semestres" className="pt-4 space-y-3">
          ...
          {loading ? (<CenteredLoader label="Cargando semestres..." className="border rounded-xl bg-card"/>) : (<div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                {semestres.map((s) => (<div key={s.id} className="border rounded-xl p-4 bg-card shadow-sm border-l-4 border-l-primary">
                    <div className="flex items-center justify-between gap-2">
                      <div className="min-w-0">
        ...
        <TabsContent value="modalidades" className="pt-4 space-y-3">
          ...
          {loading ? (<CenteredLoader label="Cargando modalidades..." className="border rounded-xl bg-card"/>) : (<div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
              {modalidades.map((m) => (<div key={m.id} className="border rounded-xl p-4 bg-card shadow-sm border-l-4 border-l-primary">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
        ...
        <TabsContent value="tipos-doc" className="pt-4 space-y-3">
          ...
          {loading ? (<CenteredLoader label="Cargando tipos de documento..." className="border rounded-xl bg-card"/>) : (<div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
              {typeDocs.map((t) => (<div key={t.id} className="border rounded-xl p-4 bg-card shadow-sm border-l-4 border-l-primary">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
        ...
        <TabsContent value="investigacion" className="pt-4 space-y-4">
          <div className="grid gap-4 lg:grid-cols-2">
            <div className="space-y-3">
              ...
              {loading ? (<CenteredLoader label="Cargando áreas..." className="border rounded-xl bg-card"/>) : (<div className="grid gap-3">
                  {areas.map((a) => (<div key={a.id} className="border rounded-xl p-4 bg-card shadow-sm border-l-4 border-l-primary">
                      <div className="flex items-center justify-between gap-2">
                        <div className="min-w-0">
            ...
            <div className="space-y-3">
              ...
              {loading ? (<CenteredLoader label="Cargando líneas..." className="border rounded-xl bg-card"/>) : (<div className="grid gap-3">
                {lineas.map((l) => (<div key={l.id} className="border rounded-xl p-4 bg-card shadow-sm border-l-4 border-l-primary">
                    <div className="flex items-center justify-between gap-2">
                      <div className="min-w-0">
