      ...
      <Card className="border-primary/10">
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            {hasActiveFilters && (<Button size="sm" variant="outline" onClick={handleClearFilters} className="text-xs bg-transparent">
                Limpiar filtros
              </Button>)}
          </div>
        </CardHeader>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
            <div>
              ...
              <Select value={gestionFilter} onValueChange={setGestionFilter}>
                ...
                <SelectContent>
                  ...
                  {GESTIONES.map((gestion) => (<SelectItem key={gestion} value={gestion}>
                      {gestion}
                    </SelectItem>))}
                </SelectContent>
              </Select>
            ...
            <div>
              ...
              <Select value={especialidadFilter} onValueChange={setEspecialidadFilter}>
                ...
                <SelectContent>
                  <SelectItem value="all">Todas</SelectItem>
                  {ESPECIALIDADES.map((esp) => (<SelectItem key={esp} value={esp}>
                      {esp}
                    </SelectItem>))}
                </SelectContent>
              </Select>
            ...
            <div>
              ...
              <Select value={semestreFilter} onValueChange={setSemestreFilter}>
                ...
                <SelectContent>
                  ...
                  {SEMESTRES.map((sem) => (<SelectItem key={sem} value={sem}>
                      {sem}º semestre
                    </SelectItem>))}
                </SelectContent>
              </Select>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <Button onClick={handleOpenNewProjectModal} className="bg-primary hover:bg-primary/90 text-white flex gap-2">
        ...
        <CardContent>
          {loading && (<div className="flex items-center gap-2 text-sm text-muted-foreground mb-4">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {estudiantes.length > 0 ? (estudiantes.map((estudiante, index) => (<tr key={estudiante.id} className="border-b hover:bg-secondary/50 dark:hover:bg-primary/5 transition-colors">
                      ...
                      <td className="py-4 px-4">
                        <Button size="sm" onClick={handleOpenNewProjectModal} className="bg-cyan-600 hover:bg-cyan-700 text-white text-xs">
                          NUEVO PROYECTO
                        </Button>
                      </td>
                    </tr>))) : (<tr>
          ...
          <div className="flex items-center justify-between mt-6 text-sm text-muted-foreground">
            ...
            <div className="flex gap-2">
              ...
              <Button variant="outline" size="sm" onClick={() => setPage((prev) => prev + 1)} disabled={pagination ? !pagination.hasNextPage : estudiantes.length < limit}>
                Siguiente
              </Button>
            </div>
          </div>
      ...
      <Dialog open={showNewProjectModal} onOpenChange={setShowNewProjectModal}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-6">
            <div className="space-y-3 border-b pb-4">
              ...
              <div className="relative">
                <div className="relative">
                  ...
                  <Input placeholder="Buscar por nombre o código..." value={searchStudentInput} onChange={(e) => handleSearchStudent(e.target.value)} onFocus={() => setShowStudentDropdown(true)} className="pl-9"/>
                </div>

                {showStudentDropdown && searchStudentInput.trim().length >= 2 && (<div className="absolute top-full left-0 right-0 mt-1 bg-white border rounded-md shadow-lg z-50 max-h-48 overflow-y-auto dark:bg-slate-900">
                    {studentSearchResults.length > 0 ? (studentSearchResults.map((estudiante) => (<button key={estudiante.id} onClick={() => handleSelectEstudiante(estudiante)} className="w-full text-left px-4 py-2 hover:bg-primary/10 border-b last:border-b-0 transition-colors">
                          <div className="font-medium">{estudiante.nombreCompleto}</div>
                          <div className="text-xs text-muted-foreground">
                            {estudiante.codEstudiante} • {estudiante.carrera}
                          </div>
                        </button>))) : (<div className="px-4 py-3 text-sm text-muted-foreground">No se encontraron estudiantes</div>)}
                  </div>)}
              </div>
            </div>

            {selectedEstudiante && (<div className="bg-primary/10 p-4 rounded-lg space-y-3">
            ...
            {selectedEstudiante && (<div className="space-y-4">
                <div>
                  <label className="text-sm font-medium mb-2 block">Título del Proyecto de Grado *</label>
                  <Input placeholder="Ingresa el título del proyecto..." value={newProject.titulo} onChange={(e) => setNewProject({ ...newProject, titulo: e.target.value })}/>
                </div>

                <div>
                  <label className="text-sm font-medium mb-2 block">Docente de Trabajo de Grado *</label>
                  <Select value={newProject.docenteTG} onValueChange={(val) => setNewProject({ ...newProject, docenteTG: val })}>
                    <SelectTrigger disabled={docentesLoading}>
                      <SelectValue placeholder="Selecciona un docente..."/>
                    </SelectTrigger>
                    <SelectContent>
                      {docentes.map((docente) => (<SelectItem key={docente.id} value={docente.id.toString()}>
                ...
                <div>
                  ...
                  <Select value={newProject.tutor} onValueChange={(val) => setNewProject({ ...newProject, tutor: val })}>
                    <SelectTrigger disabled={docentesLoading}>
                ...
                <div>
                  ...
                  <Select value={newProject.revisor1} onValueChange={(val) => setNewProject({ ...newProject, revisor1: val })}>
                    <SelectTrigger disabled={docentesLoading}>
            ...
            <div className="flex gap-2 justify-end pt-4 border-t">
              ...
              <Button onClick={handleCreateProject} disabled={formLoading || !selectedEstudiante} className="bg-primary hover:bg-primary/90 text-white">
