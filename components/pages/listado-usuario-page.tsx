...
export default function ListadoUsuarioPage() {
    ...
    useEffect(() => {
        ...
        const loadUserImages = async () => {
            const pending = usuarios.filter((usuario) => usuario.imageUrl && !userImagesRef.current[usuario.id]);
            if (!pending.length)
                return;
            await Promise.all(pending.map(async (usuario) => {
    ...
    const handleAddEspecialidad = (esp: {
        idEspecialidad: number;
        especialidad: string;
    }) => {
    ...
    const canChangePassword = () => currentUser?.roles?.some((r) => ["ADMIN", "UTIC", "ADMINISTRADOR"].includes(r.name));
    ...
    return (<div className="p-6 space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-primary mb-2 dark:text-white">Listado de Usuarios</h1>
      ...
      <Card className="border-primary/10">
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            {(searchTerm || statusFilter || roleFilter) && (<Button size="sm" variant="outline" onClick={handleClearFilters} className="text-xs bg-transparent">
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div>
              ...
              <div className="relative">
                <Input placeholder="Nombre, correo..." value={searchTerm} onChange={(e) => {
            setSearchTerm(e.target.value);
            setPage(1);
        }} className="pr-8"/>
                {searchTerm && (<button onClick={clearSearch} className="absolute right-2 top-1/2 transform -translate-y-1/2 text-muted-foreground hover:text-foreground p-1" aria-label="Limpiar búsqueda">
                    <X className="w-4 h-4"/>
                  </button>)}
              </div>
            </div>
            ...
            setPage(1);
                ...
                <SelectContent>
                  ...
                  {USER_STATUSES.map((status) => (<SelectItem key={status.value} value={status.value}>
                      {status.label}
                    </SelectItem>))}
                </SelectContent>
              </Select>
            ...
            setPage(1);
                ...
                <SelectContent>
                  <SelectItem value="ALL">Todos</SelectItem>
                  {Object.entries(ROLES_MAP).map(([key, label]) => (<SelectItem key={key} value={key}>
                      {label}
                    </SelectItem>))}
                </SelectContent>
              </Select>
            ...
            <div>
              ...
              <select className="w-full border rounded-md px-3 py-2 bg-background" value={limit} onChange={(e) => {
            setLimit(Number(e.target.value));
            setPage(1);
        }}>
                {[5, 10, 20, 50].map((opt) => (<option key={opt} value={opt}>
                    {opt} registros
                  </option>))}
              </select>
            </div>
          ...
          {error && (<div className="mt-4">
              <ErrorDisplay message={error.message} code={error.code} title="Error al cargar usuarios" onRetry={refetch} onDismiss={clearError} variant="destructive"/>
            </div>)}
        </CardContent>
      </Card>
      ...
      <Card>
        <CardContent className="pt-6">
          {loading && (<CenteredLoader label="Cargando usuarios..." className="border rounded-xl bg-card"/>)}
          {!loading && (<div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {usuarios.length > 0 ? (usuarios.map((usuario, index) => (<tr key={usuario.id} className="border-b hover:bg-secondary/50 dark:hover:bg-primary/15">
                      <td className="py-3 px-4">{(page - 1) * limit + index + 1}</td>
                      <td className="py-3 px-4">
                        <img src={getUserImageSrc(usuario)} alt={usuario.fullName || usuario.email} className="w-10 h-10 rounded-full object-cover"/>
                      </td>
                      <td className="py-3 px-4 text-sm font-medium">{usuario.fullName || usuario.email}</td>
                      <td className="py-3 px-4">
                        <div className="flex flex-wrap gap-1">
                          {usuario.roles?.length
                    ? usuario.roles.map((r) => (<Badge key={r} className="bg-primary/80 text-primary-foreground">
                                  {ROLES_MAP[r] || r}
                                </Badge>))
                    : <Badge variant="secondary">Sin rol</Badge>}
                      ...
                      <td className="py-3 px-4">
                        <Button size="sm" onClick={() => setSelectedUsuario(usuario)} className="bg-cyan-600 hover:bg-cyan-700 text-white">
                          DETALLES
                        </Button>
                      </td>
                    </tr>))) : (<tr>
          ...
          <div className="flex items-center justify-between mt-6 text-sm text-muted-foreground">
            ...
            <div className="flex gap-2">
              <Button variant="outline" size="sm" onClick={() => setPage((prev) => Math.max(1, prev - 1))} disabled={pagination ? !pagination.hasPreviousPage : page === 1}>
                Anterior
              </Button>
              <Button variant="outline" size="sm" onClick={() => setPage((prev) => prev + 1)} disabled={pagination ? !pagination.hasNextPage : usuarios.length < limit}>
                Siguiente
              </Button>
            </div>
          </div>
      ...
      <Dialog open={!!selectedUsuario} onOpenChange={() => setSelectedUsuario(null)}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            {selectedUsuario && (<div className="space-y-4 max-h-[80vh] overflow-y-auto">
              {changePasswordSuccess && (<Alert className="bg-green-50 text-green-800 border-green-200 dark:bg-green-900 dark:text-green-200">
                  <AlertDescription>Contraseña restablecida correctamente al documento de identidad (via backend).</AlertDescription>
                </Alert>)}

              
              <div className="flex justify-center mb-4">
                <img src={getUserImageSrc(selectedUsuario)} alt={selectedUsuario.fullName || selectedUsuario.email} className="w-20 h-20 rounded-lg object-cover border-2 border-primary/20"/>
              </div>
              ...
              <div className="grid gap-3">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedUsuario.roles?.length ? (selectedUsuario.roles.map((r) => (<Badge key={r} className="bg-primary/80 text-primary-foreground">
                          {ROLES_MAP[r] || r}
                        </Badge>))) : (<Badge variant="secondary">Sin rol</Badge>)}
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  {canChangeStatus && (<div className="flex items-center gap-2">
                      <Select value={statusToSet} onValueChange={setStatusToSet}>
                        <SelectTrigger className="w-40 h-9">
                          <SelectValue placeholder="Estado"/>
                        </SelectTrigger>
                        <SelectContent>
                          {USER_STATUSES.map((s) => (<SelectItem key={s.value} value={s.value}>
                              {s.label}
                            </SelectItem>))}
                        </SelectContent>
                      </Select>
                      <Button size="sm" onClick={() => handleChangeStatus(selectedUsuario, statusToSet || selectedUsuario.status)} disabled={statusUpdating || !statusToSet} className="gap-2">
              ...
              {canChangePassword() && (<Button onClick={() => handleResetPassword(selectedUsuario)} disabled={changePasswordLoading} className="w-full bg-primary hover:bg-primary/90 text-white flex gap-2 mt-4">
          ...
          <div className="grid gap-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <div>
                ...
                <div className="relative">
                  <Input type={showCreatePassword ? "text" : "password"} value={createForm.password} onChange={(e) => setCreateForm({ ...createForm, password: e.target.value })} className="pr-10"/>
                  <button type="button" onClick={() => setShowCreatePassword((prev) => !prev)} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground">
              ...
              <div>
                ...
                <Select value={createForm.role} onValueChange={(val) => setCreateForm({ ...createForm, role: val })}>
                  ...
                  <SelectContent>
                    {allowedCreateRoles.map((r) => (<SelectItem key={r} value={r}>
                        {ROLES_MAP[r] || r}
                      </SelectItem>))}
                  </SelectContent>
                </Select>
            ...
            {createForm.role === "SECRETARIA" && (<div className="grid gap-3 border rounded-lg p-3 bg-muted/30">
                <div>
                ...
                <div className="relative">
                  <Input placeholder="Buscar especialidad..." value={specSearchTerm} onChange={(e) => setSpecSearchTerm(e.target.value)}/>
                  {specLoading && (<Loader2 className="w-4 h-4 animate-spin absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground"/>)}
                </div>
                {specSearchTerm.length >= 2 && (<div className="border rounded-md bg-background shadow-sm max-h-40 overflow-y-auto p-2 space-y-1">
                    {specResults.length ? (specResults.map((esp) => {
                    ...
                    return (<button key={esp.idEspecialidad} type="button" onClick={() => handleAddEspecialidad(esp)} disabled={disabled} className={`w-full text-left rounded px-2 py-1.5 hover:bg-primary/10 ${disabled ? "opacity-60 cursor-not-allowed" : ""}`}>
                ...
                <div className="flex flex-wrap gap-2">
                  {createForm.especialidades.length ? (createForm.especialidades.map((esp) => (<Badge key={esp.idEspecialidad} className="bg-primary/10 text-primary border border-primary/20 gap-1">
                        {esp.especialidad}
                        <button type="button" onClick={() => handleRemoveEspecialidad(esp.idEspecialidad)} className="text-primary hover:text-primary/80">
                          <X className="w-3 h-3"/>
                        </button>
                      </Badge>))) : (<p className="text-sm text-muted-foreground">No hay especialidades seleccionadas</p>)}
                </div>
              </div>)}
