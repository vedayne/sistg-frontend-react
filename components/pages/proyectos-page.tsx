...
export default function ProyectosPage() {
    ...
    const openDetail = async (projectId: number) => {
        try {
            ...
            setDetailSelectedJefe(proj?.userJefeC
                ? {
        ...
        if (showCreateModal || detailOpen) {
            const loadOptions = async () => {
                try {
                    ...
                    setGestiones(gestionesData.filter((g: any) => g.isActive));
    ...
    return (<div className="p-6 space-y-6">
      <div className="flex justify-between items-center flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          {canCreate && (<Button onClick={() => setShowCreateModal(true)} className="gap-2">
      ...
      <div className="grid gap-2 md:grid-cols-3">
        <div className="md:col-span-2">
          <Input placeholder="Buscar por título o estudiante..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}/>
        </div>
      </div>

      <Card>
        <CardContent className="pt-6">
          {loading ? (<CenteredLoader label="Cargando proyectos..."/>) : (<div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                .map((p) => (<tr key={p.id} className="border-b hover:bg-muted/50">
                        ...
                        </tr>))) : (<tr><td colSpan={6} className="p-8 text-center text-muted-foreground">No se encontraron proyectos.</td></tr>)}
                </tbody>
              </table>
            </div>)}
        </CardContent>
      ...
      <Dialog open={detailOpen} onOpenChange={setDetailOpen}>
        <DialogContent className="!max-w-[58vw] !w-[58vw] max-h-[90vh] overflow-y-auto">
          ...
          {detailLoading ? (<div className="flex items-center gap-2 text-sm text-muted-foreground">
              <Loader2 className="w-4 h-4 animate-spin"/> Cargando...
            </div>) : detailProject ? (<div className="space-y-6">
              <div className="grid gap-4 lg:grid-cols-[2fr,1fr]">
                <div className="grid gap-2">
                  <Label>Título del proyecto</Label>
                  <Textarea value={detailForm.titulo} onChange={(e) => setDetailForm((prev) => ({ ...prev, titulo: e.target.value }))} className="min-h-[120px] text-base"/>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="grid gap-2">
                  ...
                  <Select value={String(detailForm.idLineaInv || "")} onValueChange={(v) => setDetailForm((prev) => ({ ...prev, idLineaInv: Number(v) }))}>
                    ...
                    <SelectContent className="w-[300px]">
                      {researchLines.map((line) => (<SelectItem key={line.id} value={String(line.id)}>
                          {line.name}
                        </SelectItem>))}
                    </SelectContent>
                  </Select>
                ...
                <div className="grid gap-2">
                  <Label>Gestión</Label>
                  <Select value={String(detailForm.idGestion || "")} onValueChange={(v) => setDetailForm((prev) => ({ ...prev, idGestion: Number(v) }))}>
                    <SelectTrigger><SelectValue placeholder="Seleccione..."/></SelectTrigger>
                    <SelectContent>
                      {gestiones.map((g) => (<SelectItem key={g.id} value={String(g.id)}>
                ...
                <div className="grid gap-2">
                  ...
                  <Select value={String(detailForm.idModalidad || "")} onValueChange={(v) => setDetailForm((prev) => ({ ...prev, idModalidad: Number(v) }))}>
                    <SelectTrigger><SelectValue placeholder="Seleccione..."/></SelectTrigger>
                    <SelectContent>
                      {modalidades.map((m) => (<SelectItem key={m.id} value={String(m.id)}>{m.name}</SelectItem>))}
                    </SelectContent>
                  </Select>
                </div>
              </div>
              ...
              <div className="grid gap-2">
                ...
                {detailSelectedJefe ? (<div className="flex justify-between items-center p-2 border rounded bg-muted/20">
                    ...
                    <Input placeholder="Buscar jefe de carrera..." className="pl-8" value={detailJefeSearchTerm} onChange={(e) => handleSearchJefeDetail(e.target.value)}/>
                    {detailJefeLoading && <Loader2 className="absolute right-2 top-2.5 h-4 w-4 animate-spin text-muted-foreground"/>}
                    {detailJefeResults.length > 0 && (<div className="absolute top-full left-0 right-0 z-10 bg-background border rounded-md shadow-lg mt-1 max-h-40 overflow-auto">
                        {detailJefeResults.map((j) => (<div key={j.id} className="p-2 hover:bg-muted cursor-pointer" onClick={() => {
              ...
              <div className="grid gap-3 border rounded-lg p-3">
                ...
                <div className="grid gap-2">
                  <div className="grid md:grid-cols-[200px,1fr] gap-3 items-center">
                    ...
                    <div className="relative">
                      <Search className="absolute left-2 top-2.5 h-4 w-4 text-muted-foreground"/>
                      <Input placeholder="Buscar por nombre, correo o código..." className="pl-8" value={detailTeacherSearchTerm} onChange={(e) => handleSearchTeacherDetail(e.target.value)}/>
                      {detailTeacherSearchLoading && (<Loader2 className="absolute right-2 top-2.5 h-4 w-4 animate-spin text-muted-foreground"/>)}
                      {detailTeacherResults.length > 0 && (<div className="absolute top-full left-0 right-0 z-10 bg-background border rounded-md shadow-lg mt-1 max-h-48 overflow-auto">
                          {detailTeacherResults.map((t) => (<div key={t.id} className="p-2 hover:bg-muted cursor-pointer text-sm" onClick={() => {
                  ...
                  {detailTeacherSearchTerm && detailTeacherResults.length === 0 && !detailTeacherSearchLoading && (<p className="text-xs text-muted-foreground">Sin resultados para “{detailTeacherSearchTerm}”.</p>)}
                </div>
                ...
                return (<div key={key} className="border rounded-md p-3 bg-muted/30 grid gap-2">
                        <div className="flex items-center justify-between gap-2">
                          <div className="text-xs text-muted-foreground">{label}</div>
                          {selected && (<Button size="icon" variant="ghost" className="h-8 w-8" onClick={() => {
            ...
            </div>) : (<p className="text-sm text-muted-foreground">No se pudo cargar el proyecto.</p>)}
          <DialogFooter>
      ...
      <Dialog open={showTemarioModal} onOpenChange={(open) => {
        ...
        <DialogContent className="!max-w-[50vw] !w-[50vw] max-h-[90vh] overflow-y-auto">
          ...
          {temarioLoading ? (<div className="flex items-center justify-center py-12">
              ...
              <div className="flex items-center justify-between gap-4">
                ...
                <Button size="sm" variant="outline" onClick={() => setTemarioCapitulos((prev) => [...prev, createTemarioCapitulo()])}>
                  <Plus className="w-4 h-4 mr-1"/>
                  Agregar capítulo
                </Button>
              </div>

              {temarioCapitulos.length === 0 && (<p className="text-sm text-muted-foreground">Aún no hay capítulos. Agrega el primero.</p>)}

              <div className="space-y-3">
                {temarioCapitulos.map((capitulo, capIndex) => (<div key={capitulo.id} className="rounded-md border p-3 space-y-3">
                    <div className="flex flex-wrap items-center gap-2">
                      <div className="text-sm font-semibold w-24">{`Capítulo ${capIndex + 1}`}</div>
                      <Input value={capitulo.titulo} onChange={(e) => {
                ...
                }} placeholder="Título del capítulo" className="flex-1"/>
                      <Button variant="ghost" size="icon" disabled={capIndex === 0} onClick={() => {
                    setTemarioCapitulos((prev) => {
                        if (capIndex === 0)
                            return prev;
                        const next = [...prev];
                      ...
                      <Button variant="ghost" size="icon" disabled={capIndex === temarioCapitulos.length - 1} onClick={() => {
                    setTemarioCapitulos((prev) => {
                        if (capIndex >= prev.length - 1)
                            return prev;
                        const next = [...prev];
                    ...
                    <div className="space-y-2 pl-4">
                      {capitulo.subcapitulos.map((sub, subIndex) => (<div key={sub.id} className="flex flex-wrap items-center gap-2">
                          <div className="text-xs text-muted-foreground w-16">
                            {capIndex + 1}.{subIndex + 1}
                          </div>
                          <Input value={sub.titulo} onChange={(e) => {
                        const value = e.target.value;
                        setTemarioCapitulos((prev) => {
                            const next = [...prev];
                    ...
                    }} placeholder="Título del subcapítulo" className="flex-1"/>
                          <Button variant="ghost" size="icon" disabled={subIndex === 0} onClick={() => {
                        setTemarioCapitulos((prev) => {
                            if (subIndex === 0)
                                return prev;
                            const next = [...prev];
                          ...
                          <Button variant="ghost" size="icon" disabled={subIndex === capitulo.subcapitulos.length - 1} onClick={() => {
                        setTemarioCapitulos((prev) => {
                            if (subIndex >= prev[capIndex].subcapitulos.length - 1)
                                return prev;
                            const next = [...prev];
                            const subs = [...next[capIndex].subcapitulos];
              ...
              <div className="space-y-2">
                ...
                <div className="grid gap-2 sm:grid-cols-2">
                  <label className="flex items-center gap-2 text-sm">
                    <Checkbox checked={temarioSecciones.bibliografia} onCheckedChange={(checked) => setTemarioSecciones((prev) => ({ ...prev, bibliografia: checked === true }))}/>
                    BIBLIOGRAFIA
                  </label>
                  <label className="flex items-center gap-2 text-sm">
                    <Checkbox checked={temarioSecciones.glosario} onCheckedChange={(checked) => setTemarioSecciones((prev) => ({ ...prev, glosario: checked === true }))}/>
                    GLOSARIO
                  </label>
                  <label className="flex items-center gap-2 text-sm">
                    <Checkbox checked={temarioSecciones.anexos} onCheckedChange={(checked) => setTemarioSecciones((prev) => ({ ...prev, anexos: checked === true }))}/>
                    ANEXOS
                  </label>
                  <label className="flex items-center gap-2 text-sm">
                    <Checkbox checked={temarioSecciones.acronimos} onCheckedChange={(checked) => setTemarioSecciones((prev) => ({ ...prev, acronimos: checked === true }))}/>
                    ACRONIMOS
                  </label>
                </div>
              </div>
          ...
          <DialogFooter>
            ...
            <Button disabled={!temarioProject || temarioSaving} onClick={async () => {
            if (!temarioProject)
                return;
            try {
        ...
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-3">
            <div className="grid gap-2">
              ...
              <Input id="nota-cite" value={detailNotaCite} onChange={(e) => setDetailNotaCite(e.target.value)} placeholder="Ej. CITE-123/2024"/>
            </div>
            <div className="grid gap-2">
              <Label htmlFor="nota-fecha">Fecha</Label>
              <Input id="nota-fecha" type="date" value={detailNotaFecha} onChange={(e) => setDetailNotaFecha(e.target.value)}/>
            </div>
            <div className="grid gap-2">
              <Label htmlFor="nota-fecha-defensa">Fecha de defensa</Label>
              <Input id="nota-fecha-defensa" type="date" value={detailNotaFechaDefensa} onChange={(e) => setDetailNotaFechaDefensa(e.target.value)}/>
            </div>
            <div className="grid gap-2">
              <Label htmlFor="nota-hora-defensa">Hora de defensa</Label>
              <Input id="nota-hora-defensa" type="time" value={detailNotaHoraDefensa} onChange={(e) => setDetailNotaHoraDefensa(e.target.value)}/>
            </div>
            <div className="grid gap-2">
              ...
              <Select value={faseSelected} onValueChange={setFaseSelected}>
                ...
                <SelectContent>
                  {phases.map((f) => (<SelectItem key={f.id} value={f.name}>
                      {f.name}
                    </SelectItem>))}
                </SelectContent>
              </Select>
      ...
      <Dialog open={showNotaEmpastadoModal} onOpenChange={setShowNotaEmpastadoModal}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-3">
            <div className="grid gap-2">
              <Label>CITE</Label>
              <Input value={notaEmpastadoCite} onChange={(e) => setNotaEmpastadoCite(e.target.value)} placeholder="Ej. CITE-EMP-123/2024"/>
            </div>
            <div className="grid gap-2">
              <Label>Ciudad</Label>
              <Input value={notaEmpastadoCiudad} onChange={(e) => setNotaEmpastadoCiudad(e.target.value)} placeholder="Ej. LA PAZ"/>
            </div>
            <div className="grid gap-2">
              <Label>Fecha</Label>
            ...
            <div className="grid gap-2">
              ...
              <Input type="date" value={notaEmpastadoFechaPresentacion} onChange={(e) => setNotaEmpastadoFechaPresentacion(e.target.value)}/>
            </div>
          </div>
          <DialogFooter className="pt-4">
      ...
      <Dialog open={showCartaPerfilModal} onOpenChange={setShowCartaPerfilModal}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-3">
            <div className="grid gap-2">
              <Label>CITE</Label>
              <Input value={cartaPerfilCite} onChange={(e) => setCartaPerfilCite(e.target.value)} placeholder="Ej. CITE-123/2024"/>
            </div>
            <div className="grid gap-2">
              <Label>Ciudad</Label>
              <Input value={cartaPerfilCiudad} onChange={(e) => setCartaPerfilCiudad(e.target.value)} placeholder="Ej. LA PAZ"/>
            </div>
            <div className="grid gap-2">
              <Label>Fecha</Label>
      ...
      <Dialog open={showInformeRevisionModal} onOpenChange={setShowInformeRevisionModal}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-3">
            <div className="grid gap-2">
              <Label>Ciudad</Label>
              <Input value={informeRevisionCiudad} onChange={(e) => setInformeRevisionCiudad(e.target.value)} placeholder="Ej. LA PAZ"/>
            </div>
            <div className="grid gap-2">
              <Label>Fecha</Label>
      ...
      <Dialog open={showTemarioDocModal} onOpenChange={setShowTemarioDocModal}>
        <DialogContent className="max-w-sm">
          ...
          <DialogFooter className="pt-4">
            ...
            <Button disabled={temarioDocGenerating} onClick={async () => {
            if (!detailProject)
                return;
          ...
          <div className="space-y-4">
            <div className="grid md:grid-cols-2 gap-3">
              ...
              <div className="grid gap-2">
                ...
                <Input type="date" value={controlFechaDevolucion} onChange={(e) => setControlFechaDevolucion(e.target.value)}/>
              </div>
              <div className="grid gap-2 md:col-span-2">
                <Label>Revision por</Label>
                <Input value={controlRevisionPor} onChange={(e) => setControlRevisionPor(e.target.value)} placeholder="Nombre del revisor"/>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="rounded-lg border">
                ...
                {controlDetalles.map((row, index) => (<div key={`detail-control-${index}`} className="grid grid-cols-[44px_minmax(0,1fr)_120px_140px_100px] gap-2 border-t text-sm items-center">
            ...
            }} placeholder="" className="h-8 border-0"/>
            ...
            <div className="space-y-2">
              ...
              <div className="rounded-lg border">
                ...
                {controlCumpleLabels.map((label, index) => (<div key={`control-cumple-${index}`} className="grid grid-cols-[44px_minmax(0,1fr)_120px_120px] gap-2 border-t px-2 py-2 text-sm items-start">
                    ...
                    <div className="flex justify-center items-center">
                      <Checkbox checked={controlCumple[index]?.cumple} onCheckedChange={(checked) => {
                const value = checked === true;
                setControlCumple((prev) => {
                    ...
                    <div className="flex justify-center items-center">
                      <Checkbox checked={controlCumple[index]?.noCumple} onCheckedChange={(checked) => {
                const value = checked === true;
                setControlCumple((prev) => {
                    const next = [...prev];
            ...
            <div className="grid gap-2">
              ...
              <Textarea value={controlObservaciones} onChange={(e) => setControlObservaciones(e.target.value)} rows={4}/>
            </div>
          </div>
          <DialogFooter className="pt-4">
            <Button variant="outline" onClick={() => setShowControlModal(false)}>Cancelar</Button>
            <Button disabled={controlGenerating} onClick={async () => {
            if (!detailProject)
                return;
      ...
      <Dialog open={showInvitacionModal} onOpenChange={setShowInvitacionModal}>
        <DialogContent className="max-w-sm">
          ...
          <DialogFooter className="pt-4">
            ...
            <Button disabled={!detailProject || invGenerating} onClick={async () => {
            if (!detailProject)
                return;
            if (!invFecha) {
      ...
      <Dialog open={showAceptacionModal} onOpenChange={setShowAceptacionModal}>
        <DialogContent className="max-w-sm">
          ...
          <DialogFooter className="pt-4">
            ...
            <Button disabled={!detailProject || aceGenerating} onClick={async () => {
            if (!detailProject)
                return;
            if (!aceFecha) {
                toast({ variant: "destructive", title: "Fecha es obligatoria" });
      ...
      <Dialog open={showActaModal} onOpenChange={setShowActaModal}>
        <DialogContent className="max-w-md">
          ...
          <DialogFooter className="pt-4">
            ...
            <Button disabled={!detailProject || actaGenerating} onClick={async () => {
            if (!detailProject) {
      ...
      <Dialog open={showMemoModal} onOpenChange={setShowMemoModal}>
        <DialogContent className="max-w-md">
          ...
          <DialogFooter className="pt-4">
            ...
            <Button disabled={!detailProject || memoGenerating} onClick={async () => {
            if (!detailProject) {
                toast({ variant: "destructive", title: "Sin proyecto cargado" });
                return;
            }
      ...
      <Dialog open={showMemoAsignacionModal} onOpenChange={setShowMemoAsignacionModal}>
        <DialogContent className="max-w-md">
          ...
          <div className="grid gap-3">
            <div className="grid gap-2">
              <Label>CITE</Label>
              <Input value={memoAsignacionCite} onChange={(e) => setMemoAsignacionCite(e.target.value)} placeholder="Ej. MEMO-001"/>
            </div>
            <div className="grid gap-2">
              <Label>Ciudad</Label>
              <Input value={memoAsignacionCiudad} onChange={(e) => setMemoAsignacionCiudad(e.target.value)} placeholder="Ej. LA PAZ"/>
            </div>
            <div className="grid gap-2">
              <Label>Fecha</Label>
          ...
          <DialogFooter className="pt-4">
            ...
            <Button disabled={!detailProject || memoAsignacionGenerating} onClick={async () => {
            if (!detailProject) {
                toast({ variant: "destructive", title: "Sin proyecto cargado" });
                return;
            }
      ...
      <Dialog open={showAvalModal} onOpenChange={setShowAvalModal}>
        <DialogContent className="max-w-md">
          ...
          <DialogFooter className="pt-4">
            ...
            <Button disabled={!detailProject || avalGenerating} onClick={async () => {
            if (!detailProject) {
                toast({ variant: "destructive", title: "Sin proyecto cargado" });
                return;
            }
        ...
        <DialogContent className="w-[95vw] max-w-[95vw] md:max-w-6xl lg:max-w-7xl max-h-[85vh] overflow-y-auto overflow-x-hidden">
          ...
          <div className="grid gap-6 py-4">
            ...
            <div className="flex flex-col gap-2">
              ...
              {isStudent && selectedStudent ? (<div className="p-2 border rounded bg-muted/20 text-sm">
                  <div className="flex items-center justify-between">
                    <div>
                ...
                </div>) : selectedStudent ? (<div className="flex justify-between items-center p-2 border rounded bg-muted/20">
                  <div className="text-sm">
                ...
                setFormData((prev) => ({ ...prev, idEstudiante: undefined }));
                  ...
                  <Search className="absolute left-2 top-2.5 h-4 w-4 text-muted-foreground"/>
                  <Input placeholder="Buscar estudiante por nombre, correo o ID SAGA..." className="pl-8" value={studentSearchTerm} onChange={e => handleSearchStudent(e.target.value)}/>
                  {searchLoading && (<Loader2 className="absolute right-2 top-2.5 h-4 w-4 animate-spin text-muted-foreground"/>)}
                  {studentSearchResults.length > 0 && (<div className="absolute top-full left-0 right-0 z-10 bg-background border rounded-md shadow-lg mt-1 max-h-48 overflow-auto">
                      {studentSearchResults.map(s => (<div key={s.id} className="p-2 hover:bg-muted cursor-pointer" onClick={() => {
                  ...
                  {studentSearchTerm.length >= 2 && !searchLoading && studentSearchResults.length === 0 && (<p className="text-xs text-muted-foreground mt-1">Sin resultados para “{studentSearchTerm}”.</p>)}
                </div>)}
            </div>
            <div className="grid gap-2">
              <label className="text-sm font-medium">Jefe de Carrera (opcional)</label>
              {selectedJefe ? (<div className="flex justify-between items-center p-2 border rounded bg-muted/20">
                  <div className="text-sm">
                ...
                setFormData((prev) => ({ ...prev, idUserJefeC: undefined }));
                  ...
                  <Search className="absolute left-2 top-2.5 h-4 w-4 text-muted-foreground"/>
                  <Input placeholder="Buscar jefe de carrera..." className="pl-8" value={jefeSearchTerm} onChange={(e) => handleSearchJefe(e.target.value)}/>
                  {jefeLoading && <Loader2 className="absolute right-2 top-2.5 h-4 w-4 animate-spin text-muted-foreground"/>}
                  {jefeResults.length > 0 && (<div className="absolute top-full left-0 right-0 z-10 bg-background border rounded-md shadow-lg mt-1 max-h-40 overflow-auto">
                      {jefeResults.map((j) => (<div key={j.id} className="p-2 hover:bg-muted cursor-pointer" onClick={() => {
                        setSelectedJefe(j);
            ...
            <div className="grid gap-2">
              ...
              <Textarea value={formData.titulo} onChange={e => setFormData({ ...formData, titulo: e.target.value })} placeholder="Ingrese el título..." className="min-h-[120px] text-base"/>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="grid gap-2">
                ...
                <Select value={String(formData.idLineaInv || "")} onValueChange={v => setFormData({ ...formData, idLineaInv: Number(v) })}>
              ...
              <div className="grid gap-2">
                ...
                <Select value={String(formData.idGestion || "")} onValueChange={v => setFormData({ ...formData, idGestion: Number(v) })}>
                  <SelectTrigger><SelectValue placeholder="Seleccione..."/></SelectTrigger>
                  <SelectContent>
                    {gestiones.map(g => (<SelectItem key={g.id} value={String(g.id)}>
                        {g.gestion} {g.typeGestion ? `(${g.typeGestion})` : ""}
              ...
              <div className="grid gap-2">
                ...
                <Select value={String(formData.idModalidad || "")} onValueChange={v => setFormData({ ...formData, idModalidad: Number(v) })}>
                  <SelectTrigger><SelectValue placeholder="Seleccione..."/></SelectTrigger>
                  <SelectContent>
                    {modalidades.map((m) => (<SelectItem key={m.id} value={String(m.id)}>{m.name}</SelectItem>))}
                  </SelectContent>
            ...
            <div className="grid gap-3 border rounded-lg p-3">
              ...
              <div className="grid gap-2">
                <div className="grid md:grid-cols-[200px,1fr] gap-3 items-center">
                  ...
                  <div className="relative">
                    <Search className="absolute left-2 top-2.5 h-4 w-4 text-muted-foreground"/>
                    <Input placeholder="Buscar por nombre, correo o código..." className="pl-8" value={teacherSearchTerm} onChange={(e) => handleSearchTeacher(e.target.value)}/>
                    {teacherSearchLoading && <Loader2 className="absolute right-2 top-2.5 h-4 w-4 animate-spin text-muted-foreground"/>}
                    {teacherResults.length > 0 && (<div className="absolute top-full left-0 right-0 z-10 bg-background border rounded-md shadow-lg mt-1 max-h-48 overflow-auto">
                        {teacherResults.map((t) => (<div key={t.id} className="p-2 hover:bg-muted cursor-pointer text-sm" onClick={() => {
                ...
                {teacherSearchTerm && teacherResults.length === 0 && !teacherSearchLoading && (<p className="text-xs text-muted-foreground">Sin resultados para “{teacherSearchTerm}”.</p>)}
              </div>
