    ...
    return (<div className="p-6 space-y-6">
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <Input placeholder="Buscar estudiante..." value={searchStudent} onChange={(e) => setSearchStudent(e.target.value)}/>
            </div>
            <div className="flex flex-col">
              <label className="block text-sm font-medium mb-1">Fase</label>
              <div className="flex gap-2">
                <select className="flex-1 px-3 py-2 border border-input rounded-lg focus:outline-none focus:ring-2 focus:ring-primary bg-background" value={filterPhaseId} onChange={(e) => setFilterPhaseId(e.target.value)}>
                  <option value="">Todas las fases</option>
                  {phases.map((fase) => (<option key={fase.id} value={fase.id}>
                      {fase.name}
                    </option>))}
                </select>
                {hasActiveFilters && (<Button onClick={handleClearFilters} variant="outline" className="text-xs bg-transparent" title="Limpiar búsqueda">
                    <X className="w-4 h-4"/>
                  </Button>)}
              </div>
            </div>
            ...
            <table className="w-full text-sm table-fixed">
              ...
              <tbody>
                {defensasLoading ? (<tr>
                    ...
                    <td colSpan={8} className="py-4 text-center text-red-600">
                  ...
                  </tr>) : filteredDefensas.length > 0 ? (filteredDefensas.map((defensa, index) => (<tr key={defensa.id} className="border-b hover:bg-secondary/50 dark:hover:bg-slate-800">
                      ...
                      <td className="py-3 px-4">
                        <Button size="sm" onClick={() => setSelectedDefensa(defensa)} className="bg-cyan-600 hover:bg-cyan-700 text-white">
                          Detalles
                        </Button>
                      </td>
                    </tr>))) : (<tr>
      ...
      <Dialog open={!!selectedDefensa} onOpenChange={() => setSelectedDefensa(null)}>
        <DialogContent className="max-w-2xl">
          ...
          {selectedDefensa && (<div className="space-y-4">
              ...
              <Button onClick={() => setSelectedDefensa(null)} className="w-full bg-primary hover:bg-primary/90 text-white">
                Cerrar
              </Button>
            </div>)}
        </DialogContent>
      ...
      <Dialog open={showNewDefensaModal} onOpenChange={(open) => {
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-hidden">
          ...
          <div className="space-y-4 max-h-[65vh] overflow-y-auto pr-2">
            <div>
              ...
              <select className="w-full px-3 py-2 border border-input rounded-lg focus:outline-none focus:ring-2 focus:ring-primary bg-background" value={newDefensa.idFase} onChange={(e) => setNewDefensa({ ...newDefensa, idFase: e.target.value })}>
            ...
            <div>
              ...
              <Input placeholder="Buscar estudiante..." value={studentSearchTerm} onChange={(e) => handleSearchStudent(e.target.value)}/>
              {studentSearchLoading && <p className="text-xs text-muted-foreground mt-1">Buscando...</p>}
              {studentSearchResults.length > 0 && (<div className="mt-2 border rounded-lg max-h-44 overflow-y-auto bg-background">
                  {studentSearchResults.map((student) => (<button key={student.id} type="button" className="w-full text-left px-3 py-2 hover:bg-secondary/60 text-sm" onClick={() => handleSelectStudent(student)}>
            ...
            {selectedStudent && (<div className="space-y-2">
                ...
                {!projectsLoading && studentProjects.length === 0 && (<p className="text-xs text-red-600">El estudiante no tiene proyectos registrados.</p>)}

                {!projectsLoading && studentProjects.length === 1 && selectedProject && (<div className="p-3 bg-secondary/10 rounded-lg border border-secondary">
                    <p className="text-sm font-medium text-muted-foreground mb-1">Proyecto</p>
                    <p className="font-medium text-foreground">{selectedProject.titulo}</p>
                  </div>)}

                {!projectsLoading && studentProjects.length > 1 && (<div>
                    <label className="block text-sm font-medium mb-2">Proyecto *</label>
                    <select className="w-full px-3 py-2 border border-input rounded-lg focus:outline-none focus:ring-2 focus:ring-primary bg-background" value={selectedProjectId} onChange={(e) => setSelectedProjectId(e.target.value)}>
                      <option value="">Selecciona un proyecto</option>
                      {studentProjects.map((proj) => (<option key={proj.id} value={proj.id}>
                          {proj.titulo}
                        </option>))}
                    </select>
                  </div>)}
            ...
            <div>
              ...
              <Input type="text" placeholder="Ej: 18.5" value={newDefensa.notaReferencial} onChange={(e) => setNewDefensa({ ...newDefensa, notaReferencial: e.target.value })}/>
            </div>

            <div>
              <label className="block text-sm font-medium mb-2">Observación</label>
              <textarea className="w-full px-3 py-2 border border-input rounded-lg focus:outline-none focus:ring-2 focus:ring-primary bg-background" placeholder="Ingresa observaciones..." value={newDefensa.observaciones} onChange={(e) => setNewDefensa({ ...newDefensa, observaciones: e.target.value })} rows={4}/>
            </div>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSaveNewDefensa} className="bg-green-600 hover:bg-green-700 text-white" disabled={createLoading}>
