...
Para realizar cambios, consulte con su Docente de Trabajo de Grado.
    ...
    return (<div className="p-6 max-w-6xl mx-auto space-y-6">
      ...
      <Tabs defaultValue="temario" className="w-full">
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {MODALITIES.map((mod) => (<button key={mod.id} onClick={() => setSelectedModality(mod.id as GraduationModality)} disabled={temarioGuardado} className={`p-3 rounded-lg border-2 transition-all text-center ${selectedModality === mod.id
                ? "border-primary bg-primary/10"
                : "border-border hover:border-primary/50"} ${temarioGuardado ? "opacity-50 cursor-not-allowed" : ""}`}>
          ...
          {!temarioGuardado ? (<Card>
              ...
              <CardContent className="space-y-6">
                <div>
                  ...
                  <Input placeholder="Ingresa el título de tu trabajo de grado" value={temarioData.titulo} onChange={(e) => setTemarioData({ ...temarioData, titulo: e.target.value })}/>
                </div>

                <div>
                  <label className="block text-sm font-medium mb-2">Objetivos</label>
                  <Textarea placeholder="Describe los objetivos de tu trabajo" value={temarioData.objetivos} onChange={(e) => setTemarioData({ ...temarioData, objetivos: e.target.value })} rows={3}/>
                </div>

                <div>
                  <label className="block text-sm font-medium mb-2">Alcance</label>
                  <Textarea placeholder="Define el alcance de tu trabajo" value={temarioData.alcance} onChange={(e) => setTemarioData({ ...temarioData, alcance: e.target.value })} rows={3}/>
                </div>

                <div>
                  <label className="block text-sm font-medium mb-2">Metodología</label>
                  <Textarea placeholder="Describe la metodología a utilizar" value={temarioData.metodologia} onChange={(e) => setTemarioData({ ...temarioData, metodologia: e.target.value })} rows={3}/>
                </div>

                {selectedModality !== "trabajo-dirigido" && (<div>
                    <label className="block text-sm font-medium mb-2">Estructura de Capítulos</label>
                    <Textarea placeholder="Describe la estructura de capítulos de tu trabajo" value={temarioData.capitulos} onChange={(e) => setTemarioData({ ...temarioData, capitulos: e.target.value })} rows={4}/>
                  </div>)}
              ...
              <Card>
                ...
                if (pdfUrl) {
                    ...
                    <Button onClick={() => setShowPdfPreview(!showPdfPreview)} variant="outline" className="flex items-center gap-2">
                      <Eye className="w-4 h-4"/>
                      {showPdfPreview ? "Ocultar" : "Ver"} Vista Previa
                    </Button>
                  </div>

                  {showPdfPreview && pdfUrl && (<div className="border rounded-lg p-4 bg-slate-50">
        ...
        <TabsContent value="modalidades" className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {MODALITIES.map((mod) => (<Card key={mod.id}>
                <CardHeader>
      ...
      {showWarningModal && (<div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
