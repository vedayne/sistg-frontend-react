      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          ...
          <CardContent className="space-y-3">
            <Input placeholder="Nombre o correo (mín. 2 letras)" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}/>
            {searchTerm.length >= 2 && (<div className="border rounded-md max-h-64 overflow-y-auto">
                {loadingSearch && (<div className="flex items-center gap-2 px-3 py-2 text-muted-foreground text-sm">
                    <Loader2 className="w-4 h-4 animate-spin"/> Buscando...
                  </div>)}
                {!loadingSearch && results.length === 0 && (<p className="px-3 py-2 text-sm text-muted-foreground">Sin resultados</p>)}
                {!loadingSearch &&
                results.map((u) => (<button key={u.id} type="button" onClick={() => setSelectedUser(u)} className={`w-full text-left px-3 py-2 hover:bg-primary/10 ${selectedUser?.id === u.id ? "bg-primary/10" : ""}`}>
                      <p className="text-sm font-semibold">{u.fullName || u.email}</p>
                      <p className="text-xs text-muted-foreground break-all">{u.email}</p>
                      {u.roles?.length ? (<p className="text-[11px] text-muted-foreground">Roles: {u.roles.join(", ")}</p>) : (<p className="text-[11px] text-muted-foreground">Sin roles</p>)}
                    </button>))}
              </div>)}
          </CardContent>
        </Card>
            ...
            {selectedUser ? (<>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 mt-1">
                    {selectedUser.roles?.length ? (selectedUser.roles.map((r) => (<div key={r} className="flex items-center gap-1">
                          <Badge>{r}</Badge>
                          <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => handleRemoveRole(r)} disabled={removingRole === r} title="Quitar rol">
                            {removingRole === r ? <Loader2 className="w-3 h-3 animate-spin"/> : "✕"}
                          </Button>
                        </div>))) : (<Badge variant="secondary">Sin roles</Badge>)}
                  </div>
                </div>
                <div className="grid gap-2">
                  ...
                  <Select value={selectedRole} onValueChange={setSelectedRole}>
                    ...
                    <SelectContent>
                      {roles
                .filter((r) => r.isActive)
                .map((r) => (<SelectItem key={r.id} value={r.name}>
                            {r.name}
                          </SelectItem>))}
                    </SelectContent>
                  </Select>
