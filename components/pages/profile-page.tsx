...
export default function ProfilePage() {
    const { user, loading, updatePassword, fetchSessions, logoutAllDevices, logoutDevice, uploadProfileImage, refreshProfile, sessions, } = useAuth();
    ...
    const userType: "estudiante" | "docente" | "administrativo" = user.academico?.codAlumno ? "estudiante" : user.academico?.codDocente ? "docente" : "administrativo";
    ...
    const semestreActual = (() => {
        const semestre = user.academico?.semestreActual;
        if (typeof semestre === "string" || semestre === null || semestre === undefined)
            return semestre;
    ...
    const parseUserAgent = (ua: string) => {
        ...
        if (isChromeMatch)
            browser = `Chrome ${isChromeMatch[1]}`;
        else if (isFirefoxMatch)
            browser = `Firefox ${isFirefoxMatch[1]}`;
        else if (isSafariMatch)
            browser = `Safari ${isSafariMatch[1]}`;
        let os = "SO desconocido";
        if (isWindowsMatch)
            os = "Windows";
        else if (isMacMatch)
            os = "macOS";
        else if (isLinuxMatch)
            os = "Linux";
        return { browser, os };
    };
    ...
    return (<div className="min-h-screen bg-background p-6 max-w-6xl mx-auto space-y-6">
      <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as "perfil" | "sesiones")} className="w-full">
        ...
        <TabsContent value="perfil" className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <Card className="md:col-span-1">
              ...
              <CardContent className="text-center space-y-4">
                <ProfileImage user={user} className="w-40 h-40 rounded-lg object-cover border-4 border-primary/20" containerClassName="relative inline-block" overlayClassName="rounded-lg" showLoader>
                ...
                <Button onClick={() => setShowPasswordDialog(true)} className="w-full bg-primary hover:bg-primary/90 text-white flex gap-2">
          ...
          <Card>
            ...
            : user.academico?.codAlumno || user.academico?.idSaga || "N/A"}
                ...
                {userType === "estudiante" && (<div className="space-y-2">
                    <label className="block text-sm font-medium text-muted-foreground">Semestre Actual</label>
                    <div className="flex flex-col sm:flex-row gap-2">
                      <Select value={selectedSemestreId} onValueChange={(value) => {
                setSelectedSemestreId(value);
                setSemestreError("");
                setSemestreMessage("");
            }} disabled={isLoadingSemestres || isUpdatingSemestre}>
                        <SelectTrigger className="w-full sm:w-52">
                          <SelectValue placeholder={isLoadingSemestres ? "Cargando..." : semestreActual || "Selecciona semestre"}/>
                        </SelectTrigger>
                        <SelectContent>
                          {semestres.length > 0 ? (semestres.map((sem) => (<SelectItem key={sem.id} value={String(sem.id)}>
                                {sem.name} ({sem.code})
                      ...
                      <Button onClick={handleUpdateSemestre} disabled={isUpdatingSemestre ||
                !selectedSemestreId ||
                ...
                {userType === "estudiante" && (<div className="space-y-2">
                    ...
                    <div className="flex flex-col sm:flex-row gap-2">
                      <Select value={selectedGestionType} onValueChange={(value) => {
                ...
                setGestionMessage("");
                      ...
                      <Button onClick={handleUpdateGestion} disabled={isUpdatingGestion || !selectedGestionType || selectedGestionType === gestionActual}>
        ...
        <TabsContent value="sesiones" className="space-y-6">
          <div className="flex flex-col md:flex-row md:flex-wrap items-start md:items-center justify-between gap-3">
            ...
            <div className="flex gap-2 w-full md:w-auto">
              <Button variant="outline" onClick={fetchSessionsWithLoading} disabled={isFetchingSessions} className="bg-secondary/10 border-secondary/30 flex-1 md:flex-none">
                {isFetchingSessions ? "Actualizando..." : "Actualizar"}
              </Button>
              {otherSessions.length > 0 && (<Button onClick={handleLogoutAllDevices} disabled={isLoggingOut || isFetchingSessions} variant="destructive" className="flex gap-2 items-center flex-1 md:flex-none">
          ...
          {isFetchingSessions ? (<div className="flex items-center justify-center py-12">
            ...
            </div>) : sessions.length === 0 ? (<Alert>
              <AlertDescription>No hay sesiones activas registradas</AlertDescription>
            </Alert>) : (<div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <CardContent className="space-y-3">
                  {currentSession ? (<>
                      ...
                      <p className="text-xs text-muted-foreground">
                        ...
                        <Button size="sm" variant="ghost" className="h-7 px-2 ml-2 text-xs" onClick={() => {
                    ...
                    setTimeout(() => setCopiedSessionId(null), 2000);
                      ...
                      {copiedSessionId === currentSession.id && (<p className="text-xs text-green-600 dark:text-green-400">Copiado al portapapeles</p>)}
                    </>) : null}
                </CardContent>
              </Card>

              {otherSessions.length > 0 && (<Card className="border-orange-200/50 dark:border-orange-900/30 shadow-sm">
                  <CardHeader>
                  ...
                  <CardContent className="space-y-3 max-h-96 overflow-y-auto">
                    ...
                    return (<div key={session.id} className="border rounded-lg p-4 space-y-2 bg-secondary/10 hover:bg-secondary/20 transition-colors">
                          <div className="flex items-start justify-between gap-4">
                            ...
                            setTimeout(() => setCopiedSessionId(null), 2000);
                        }} variant="ghost" className="h-8 px-2 text-xs text-muted-foreground hover:text-foreground" title="Copiar ID de sesión">
                                <Copy className="w-3 h-3"/>
                              </Button>
                              <Button size="sm" variant="destructive" className="h-8 px-2" onClick={() => handleLogoutDevice(session.id)} title="Cerrar sesión en este dispositivo" disabled={isLoggingOut}>
                                {isLoggingOut ? "..." : "Cerrar"}
                              </Button>
                            </div>
                          </div>
                          {copiedSessionId === session.id && (<p className="text-xs text-green-600 dark:text-green-400">Copiado al portapapeles</p>)}
                        </div>);
                })}
                  </CardContent>
                </Card>)}
      ...
      {showPasswordDialog && (<Dialog open={showPasswordDialog} onOpenChange={setShowPasswordDialog}>
          <DialogContent className="max-w-md">
            <DialogHeader>
              <DialogTitle className="text-primary">Cambiar Contraseña</DialogTitle>
            </DialogHeader>
            <div className="space-y-4">
              {passwordError && (<Alert className="bg-red-50 text-red-800 border-red-200">
                  <AlertDescription>{passwordError}</AlertDescription>
                </Alert>)}
              {passwordSuccess && (<Alert className="bg-green-50 text-green-800 border-green-200">
              ...
              <div>
                ...
                <div className="relative">
                  <Input type={showPasswords.current ? "text" : "password"} placeholder="Ingresa tu contraseña actual" value={passwordData.current} onChange={(e) => setPasswordData({ ...passwordData, current: e.target.value })} disabled={isSubmittingPassword}/>
                  <button type="button" onClick={() => setShowPasswords({ ...showPasswords, current: !showPasswords.current })} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground">
                    {showPasswords.current ? (<svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div>
                ...
                <div className="relative">
                  <Input type={showPasswords.new ? "text" : "password"} placeholder="Ingresa tu nueva contraseña" value={passwordData.new} onChange={(e) => setPasswordData({ ...passwordData, new: e.target.value })} disabled={isSubmittingPassword}/>
                  <button type="button" onClick={() => setShowPasswords({ ...showPasswords, new: !showPasswords.new })} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground">
                    {showPasswords.new ? (<svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-4.803m5.596-3.856a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                      </svg>) : (<svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
              ...
              <div>
                ...
                <div className="relative">
                  <Input type={showPasswords.confirm ? "text" : "password"} placeholder="Confirma tu nueva contraseña" value={passwordData.confirm} onChange={(e) => setPasswordData({ ...passwordData, confirm: e.target.value })} disabled={isSubmittingPassword}/>
                  <button type="button" onClick={() => setShowPasswords({ ...showPasswords, confirm: !showPasswords.confirm })} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground">
                    {showPasswords.confirm ? (<svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-4.803m5.596-3.856a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                      </svg>) : (<svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
              ...
              <div className="flex gap-3 pt-4">
                <Button onClick={handleChangePassword} disabled={isSubmittingPassword} className="flex-1 bg-primary hover:bg-primary/90">
