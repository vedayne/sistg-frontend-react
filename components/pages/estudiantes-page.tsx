    ...
    return (<div className="p-4 md:p-6 space-y-6">
          ...
          <div className="flex gap-4 items-end">
            <div className="flex-1">
              ...
              <div className="relative">
                ...
                <Input placeholder="Nombre, código o SAGA..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-9"/>
              </div>
            </div>
      ...
      <Card>
        <CardContent className="pt-6">
          {loading ? (<CenteredLoader label="Cargando estudiantes..."/>) : error ? (<div className="p-4 text-center text-red-500 bg-red-50 rounded-lg">{error}</div>) : (<div className="overflow-x-auto">
              <table className="w-full text-xs md:text-sm">
                ...
                <tbody>
                  {filteredEstudiantes.length > 0 ? (filteredEstudiantes.map((est) => (<tr key={est.id} className="border-b hover:bg-muted/50">
          ...
          {!loading && filteredEstudiantes.length > 0 && (<div className="mt-4 text-xs text-muted-foreground text-center">
