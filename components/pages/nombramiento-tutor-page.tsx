        ...
        <CardContent className="space-y-6">
          ...
          <div>
            ...
            <div className="relative">
              <Input type="date" value={selectedDate} onChange={(e) => setSelectedDate(e.target.value)} className="pr-10"/>
          ...
          <div className="space-y-3">
            ...
            <div className="relative">
              <Input placeholder="Buscar tutor por nombre o email..." value={tutorSearch} onChange={(e) => setTutorSearch(e.target.value)} className="mb-2"/>
              {isLoadingTutors && (<div className="absolute top-full left-0 mt-1 text-xs text-muted-foreground flex items-center gap-2">
                  <Loader2 className="w-3 h-3 animate-spin"/>
                  Buscando docentes...
                </div>)}
              {tutorError && <p className="text-xs text-red-600 mt-1">{tutorError}</p>}
              {tutorSearch && (<div className="absolute top-full left-0 right-0 border rounded-lg bg-card shadow-lg z-10 max-h-48 overflow-y-auto">
                  {filteredTutors.length > 0 ? (filteredTutors.map((tutor) => (<button key={tutor.id} onClick={() => {
                    ...
                    setTutorSearch("");
                      ...
                      </button>))) : (<div className="px-4 py-3 text-sm text-muted-foreground">No se encontraron docentes</div>)}
                </div>)}
            </div>

            {selectedTutor && (<div className="border rounded-lg p-4 bg-secondary/15 dark:bg-primary/20">
            ...
            <Button onClick={() => setShowExternalTutorModal(true)} variant="outline" className="w-full gap-2 bg-transparent">
          ...
          <div>
            ...
            <Input placeholder="Ingresa el título de tu proyecto..." value={projectTitle} onChange={(e) => setProjectTitle(e.target.value)}/>
          </div>

          
          <div className="flex gap-3 pt-4 border-t">
            <Button onClick={() => setIsEditing(!isEditing)} variant="outline" className="flex gap-2 flex-1 bg-transparent">
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4">
            <div>
              ...
              <Input placeholder="Nombre del tutor externo" value={externalTutorData.nombre} onChange={(e) => setExternalTutorData({ ...externalTutorData, nombre: e.target.value })}/>
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">Correo Electrónico</label>
              <Input type="email" placeholder="correo@example.com" value={externalTutorData.email} onChange={(e) => setExternalTutorData({ ...externalTutorData, email: e.target.value })}/>
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">Teléfono</label>
              <Input placeholder="+591 7xxxxxxxx" value={externalTutorData.telefono} onChange={(e) => setExternalTutorData({ ...externalTutorData, telefono: e.target.value })}/>
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">Especialidad</label>
              <Input placeholder="Especialidad del tutor" value={externalTutorData.especialidad} onChange={(e) => setExternalTutorData({ ...externalTutorData, especialidad: e.target.value })}/>
            </div>
            <div className="flex gap-2 pt-4">
              <Button onClick={handleAddExternalTutor} className="flex-1 bg-primary hover:bg-primary/90 text-white">
                Guardar
              </Button>
              <Button onClick={() => setShowExternalTutorModal(false)} variant="outline" className="flex-1 bg-transparent">
                Cancelar
              </Button>
            </div>
          </div>
