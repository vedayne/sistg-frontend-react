...
export default function GenerarReportePage() {
    ...
    return (<div className="p-6 space-y-6">
      ...
      {successMessage && (<Alert className="bg-green-50 border-green-200 dark:bg-green-900/20 dark:border-green-700">
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {TIPOS_REPORTES.map((reporte) => (<button key={reporte.id} onClick={() => setSelectedReport(reporte.id)} className={`p-4 rounded-lg border-2 transition-all text-left hover:shadow-md ${selectedReport === reporte.id
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input type="date" className="w-full px-3 py-2 border border-input rounded-lg focus:outline-none focus:ring-2 focus:ring-primary bg-background" value={fechaInicio} onChange={(e) => setFechaInicio(e.target.value)}/>
            </div>
            <div>
              <label className="block text-sm font-medium mb-2">Fecha de Fin</label>
              <input type="date" className="w-full px-3 py-2 border border-input rounded-lg focus:outline-none focus:ring-2 focus:ring-primary bg-background" value={fechaFin} onChange={(e) => setFechaFin(e.target.value)}/>
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Button onClick={() => handleGenerateReport("pdf")} disabled={isGenerating || !selectedReport} className="bg-red-600 hover:bg-red-700 text-white flex gap-2">
              <FileDown className="w-4 h-4"/>
              {isGenerating ? "Generando..." : "Generar PDF"}
            </Button>
            <Button onClick={() => handleGenerateReport("excel")} disabled={isGenerating || !selectedReport} className="bg-green-600 hover:bg-green-700 text-white flex gap-2">
