...
const buildControlDetalles = () => Array.from({ length: 12 }, () => ({
    detalle: "",
    presentaObservacion: false,
    observacionSubsanada: false,
    conforme: false,
}));
const buildControlCumple = () => Array.from({ length: 4 }, () => ({
    cumple: false,
    noCumple: false,
}));
...
export default function DocumentacionPage() {
    ...
    return (<div className="p-6 space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          {summaryError && (<div className="bg-red-50 text-red-700 border border-red-200 rounded-md p-3 text-sm mb-4">
              {summaryError}
            </div>)}
          {summaryLoading ? (<div className="flex justify-center py-8">
              <Loader2 className="w-8 h-8 animate-spin text-primary"/>
            </div>) : summary && summary.tipos.length > 0 ? (<div className="space-y-2 mb-4 text-sm text-muted-foreground">
              <span>Total tipos: {summary.totalTipos}</span> • <span>Total archivos: {summary.totalArchivos}</span>
            </div>) : (<div className="text-muted-foreground text-sm">No hay tipos de documentos disponibles.</div>)}

          {summary && summary.tipos.length > 0 && (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {summary.tipos.map((tipo) => (<Card key={tipo.tipoDocumentoId} className="hover:shadow-lg transition-all cursor-pointer border-l-4 border-l-primary" onClick={() => fetchFilesByType(tipo)}>
      ...
      <Dialog open={showFilesModal} onOpenChange={handleFilesModalOpenChange}>
        <DialogContent className="max-w-7xl w-[98vw] max-h-[85vh] overflow-hidden">
          ...
          {selectedType && (<p className="text-xs text-muted-foreground">
              Total: {selectedType.cantidadArchivos} archivo(s)
            </p>)}
          {filesLoading ? (<CenteredLoader label="Cargando archivos..."/>) : files.length === 0 ? (<p className="text-sm text-muted-foreground">No hay archivos registrados para este tipo.</p>) : (<div className="max-h-[65vh] overflow-y-auto overflow-x-auto">
              <table className="w-full text-sm table-fixed">
                ...
                <tbody>
                  {files.map((file) => (<tr key={file.id} className="border-b last:border-0">
                    ...
                    try {
                        ...
                        toast({ variant: "destructive", title: "Error", description: msg });
                          ...
                          <Button size="sm" variant="ghost" className="text-primary" onClick={() => downloadArchivo(file.id, file.originalName)}>
        ...
        <DialogContent className="max-w-xl">
          ...
          <div className="grid md:grid-cols-2 gap-3">
            <div className="grid gap-2">
              ...
              <Input type="number" value={notaForm.idProyecto} onChange={(e) => setNotaForm({ ...notaForm, idProyecto: e.target.value })} placeholder="ID del proyecto"/>
            </div>
            <div className="grid gap-2">
              <Label>CITE</Label>
              <Input value={notaForm.cite} onChange={(e) => setNotaForm({ ...notaForm, cite: e.target.value })} placeholder="Ej. CITE-123/2024"/>
            </div>
            <div className="grid gap-2">
              <Label>Fecha</Label>
              <Input type="date" value={notaForm.fecha} onChange={(e) => setNotaForm({ ...notaForm, fecha: e.target.value })}/>
            </div>
            <div className="grid gap-2">
              <Label>Fecha de defensa</Label>
              <Input type="date" value={notaForm.fechaDefensa} onChange={(e) => setNotaForm({ ...notaForm, fechaDefensa: e.target.value })}/>
            </div>
            <div className="grid gap-2">
              <Label>Hora de defensa</Label>
              <Input type="time" value={notaForm.horaDefensa} onChange={(e) => setNotaForm({ ...notaForm, horaDefensa: e.target.value })}/>
            </div>
            <div className="grid gap-2">
              <Label>Fase (opcional)</Label>
              <Select value={notaForm.fase} onValueChange={(value) => setNotaForm({ ...notaForm, fase: value })} disabled={phasesLoading || phases.length === 0}>
                <SelectTrigger>
                  <SelectValue placeholder={phasesLoading ? "Cargando fases..." : "Selecciona fase"}/>
                </SelectTrigger>
                <SelectContent>
                  {phases.map((fase) => (<SelectItem key={fase.id} value={fase.name}>
                      {fase.name}
                    </SelectItem>))}
                </SelectContent>
              </Select>
              {!phasesLoading && phases.length === 0 && (<p className="text-xs text-muted-foreground">No hay fases registradas. Crea una en Configuraciones.</p>)}
            </div>
          ...
          <DialogFooter className="pt-4">
            ...
            <Button disabled={notaLoading} onClick={async () => {
      ...
      <Dialog open={showNotaEmpastadoModal} onOpenChange={setShowNotaEmpastadoModal}>
        <DialogContent className="max-w-xl">
          ...
          <div className="grid md:grid-cols-2 gap-3">
            <div className="grid gap-2">
              <Label>ID Proyecto</Label>
              <Input type="number" value={notaEmpastadoForm.idProyecto} onChange={(e) => setNotaEmpastadoForm({ ...notaEmpastadoForm, idProyecto: e.target.value })} placeholder="ID del proyecto"/>
            </div>
            <div className="grid gap-2">
              <Label>CITE</Label>
              <Input value={notaEmpastadoForm.cite} onChange={(e) => setNotaEmpastadoForm({ ...notaEmpastadoForm, cite: e.target.value })} placeholder="Ej. CITE-123/2024"/>
            </div>
            <div className="grid gap-2">
              <Label>Ciudad</Label>
              <Input value={notaEmpastadoForm.ciudad} onChange={(e) => setNotaEmpastadoForm({ ...notaEmpastadoForm, ciudad: e.target.value })} placeholder="Ej. LA PAZ"/>
            </div>
            <div className="grid gap-2">
              <Label>Fecha</Label>
              <Input type="date" value={notaEmpastadoForm.fecha} onChange={(e) => setNotaEmpastadoForm({ ...notaEmpastadoForm, fecha: e.target.value })}/>
            </div>
            <div className="grid gap-2">
              <Label>Fecha límite de presentación</Label>
              <Input type="date" value={notaEmpastadoForm.fechaPresentacion} onChange={(e) => setNotaEmpastadoForm({ ...notaEmpastadoForm, fechaPresentacion: e.target.value })}/>
            </div>
            <div className="col-span-full text-xs text-muted-foreground">
          ...
          <DialogFooter className="pt-4">
            ...
            <Button disabled={notaEmpastadoLoading} onClick={async () => {
      ...
      <Dialog open={showCartaPerfilModal} onOpenChange={setShowCartaPerfilModal}>
        <DialogContent className="max-w-xl">
          ...
          <div className="grid md:grid-cols-2 gap-3">
            <div className="grid gap-2">
              <Label>ID Proyecto</Label>
              <Input type="number" value={cartaPerfilForm.idProyecto} onChange={(e) => setCartaPerfilForm({ ...cartaPerfilForm, idProyecto: e.target.value })} placeholder="ID del proyecto"/>
            </div>
            <div className="grid gap-2">
              <Label>CITE</Label>
              <Input value={cartaPerfilForm.cite} onChange={(e) => setCartaPerfilForm({ ...cartaPerfilForm, cite: e.target.value })} placeholder="Ej. CITE-123/2024"/>
            </div>
            <div className="grid gap-2">
              <Label>Ciudad</Label>
              <Input value={cartaPerfilForm.ciudad} onChange={(e) => setCartaPerfilForm({ ...cartaPerfilForm, ciudad: e.target.value })} placeholder="Ej. LA PAZ"/>
            </div>
            <div className="grid gap-2">
              <Label>Fecha</Label>
              <Input type="date" value={cartaPerfilForm.fecha} onChange={(e) => setCartaPerfilForm({ ...cartaPerfilForm, fecha: e.target.value })}/>
            </div>
            <div className="grid gap-2">
              <Label>Fase</Label>
              <Select value={cartaPerfilForm.fase} onValueChange={(value) => setCartaPerfilForm({ ...cartaPerfilForm, fase: value })} disabled={phasesLoading || phases.length === 0}>
                <SelectTrigger>
                  <SelectValue placeholder={phasesLoading ? "Cargando fases..." : "Selecciona fase"}/>
                </SelectTrigger>
                <SelectContent>
            ...
            <div className="grid gap-2">
              ...
              <Input value={cartaPerfilForm.anexos} onChange={(e) => setCartaPerfilForm({ ...cartaPerfilForm, anexos: e.target.value })} placeholder="S/A"/>
            </div>
            <div className="col-span-full text-xs text-muted-foreground">
          ...
          <DialogFooter className="pt-4">
            ...
            <Button disabled={cartaPerfilLoading} onClick={async () => {
      ...
      <Dialog open={showInformeRevisionModal} onOpenChange={setShowInformeRevisionModal}>
        <DialogContent className="max-w-xl">
          ...
          <div className="grid md:grid-cols-2 gap-3">
            <div className="grid gap-2">
              <Label>ID Proyecto</Label>
              <Input type="number" value={informeRevisionForm.idProyecto} onChange={(e) => setInformeRevisionForm({ ...informeRevisionForm, idProyecto: e.target.value })} placeholder="ID del proyecto"/>
            </div>
            <div className="grid gap-2">
              <Label>Ciudad</Label>
              <Input value={informeRevisionForm.ciudad} onChange={(e) => setInformeRevisionForm({ ...informeRevisionForm, ciudad: e.target.value })} placeholder="Ej. LA PAZ"/>
            </div>
            <div className="grid gap-2">
              <Label>Fecha</Label>
              <Input type="date" value={informeRevisionForm.fecha} onChange={(e) => setInformeRevisionForm({ ...informeRevisionForm, fecha: e.target.value })}/>
            </div>
            <div className="grid gap-2">
              <Label>Fase</Label>
              <Select value={informeRevisionForm.fase} onValueChange={(value) => setInformeRevisionForm({ ...informeRevisionForm, fase: value })} disabled={phasesLoading || phases.length === 0}>
                <SelectTrigger>
                  <SelectValue placeholder={phasesLoading ? "Cargando fases..." : "Selecciona fase"}/>
                </SelectTrigger>
                <SelectContent>
          ...
          <DialogFooter className="pt-4">
            ...
            <Button disabled={informeRevisionLoading} onClick={async () => {
      ...
      <Dialog open={showTemarioModal} onOpenChange={setShowTemarioModal}>
        <DialogContent className="max-w-xl">
          ...
          <div className="grid md:grid-cols-2 gap-3">
            <div className="grid gap-2">
              <Label>ID Proyecto</Label>
              <Input type="number" value={temarioForm.idProyecto} onChange={(e) => setTemarioForm({ ...temarioForm, idProyecto: e.target.value })} placeholder="ID del proyecto"/>
            </div>
            <div className="grid gap-2">
              <Label>CITE</Label>
              <Input value={temarioForm.cite} onChange={(e) => setTemarioForm({ ...temarioForm, cite: e.target.value })} placeholder="Ej. CITE-123/2025"/>
            </div>
            <div className="grid gap-2 md:col-span-2">
              <Label>Objeto</Label>
              <Input value={temarioForm.objeto} onChange={(e) => setTemarioForm({ ...temarioForm, objeto: e.target.value })} placeholder="Ej. Aprobación de temario"/>
            </div>
            <div className="grid gap-2">
              <Label>Anexos</Label>
              <Input value={temarioForm.anexos} onChange={(e) => setTemarioForm({ ...temarioForm, anexos: e.target.value })} placeholder="S/A"/>
            </div>
            <div className="grid gap-2">
              <Label>Ciudad</Label>
              <Input value={temarioForm.ciudad} onChange={(e) => setTemarioForm({ ...temarioForm, ciudad: e.target.value })} placeholder="Ej. LA PAZ"/>
            </div>
            <div className="grid gap-2">
              <Label>Fecha</Label>
              <Input type="date" value={temarioForm.fecha} onChange={(e) => setTemarioForm({ ...temarioForm, fecha: e.target.value })}/>
            </div>
            <div className="col-span-full text-xs text-muted-foreground">
          ...
          <DialogFooter className="pt-4">
            ...
            <Button disabled={temarioLoading} onClick={async () => {
        ...
        <DialogContent className="max-w-[21.59cm] w-[95vw] max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            <div className="grid md:grid-cols-2 gap-3">
              <div className="grid gap-2">
                <Label>ID Proyecto</Label>
                <Input type="number" value={controlForm.idProyecto} onChange={(e) => setControlForm({ ...controlForm, idProyecto: e.target.value })} placeholder="ID del proyecto"/>
              </div>
              <div className="grid gap-2">
                <Label>Fase</Label>
                <Select value={controlForm.fase} onValueChange={(value) => setControlForm({ ...controlForm, fase: value })} disabled={phasesLoading || phases.length === 0}>
                  <SelectTrigger>
                    <SelectValue placeholder={phasesLoading ? "Cargando fases..." : "Selecciona fase"}/>
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div className="grid gap-2">
                ...
                <Input value={controlForm.revisionPor} onChange={(e) => setControlForm({ ...controlForm, revisionPor: e.target.value })} placeholder="Nombre del revisor"/>
              </div>
              <div className="grid gap-2">
                <Label>Fecha de devolucion</Label>
                <Input type="date" value={controlForm.fechaDevolucion} onChange={(e) => setControlForm({ ...controlForm, fechaDevolucion: e.target.value })}/>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="rounded-lg border">
                ...
                {controlForm.detalles.map((row, index) => (<div key={`control-detalle-${index}`} className="grid grid-cols-[44px_minmax(0,1fr)_120px_140px_100px] gap-2 border-t px-2 py-2 text-sm items-center">
                    <div className="text-center">{index + 1}</div>
                    <Input value={row.detalle} onChange={(e) => {
            ...
            }} placeholder="Detalle" className="h-8"/>
                    <div className="flex justify-center items-center">
                      <Checkbox checked={row.presentaObservacion} onCheckedChange={(checked) => {
                ...
                setControlForm((prev) => {
                    ...
                    <div className="flex justify-center items-center">
                      <Checkbox checked={row.observacionSubsanada} onCheckedChange={(checked) => {
                const value = checked === true;
                setControlForm((prev) => {
                    const detalles = [...prev.detalles];
                    ...
                    <div className="flex justify-center items-center">
                      <Checkbox checked={row.conforme} onCheckedChange={(checked) => {
                const value = checked === true;
                setControlForm((prev) => {
                    const detalles = [...prev.detalles];
            ...
            <div className="space-y-2">
              ...
              <div className="rounded-lg border">
                ...
                {controlCumpleLabels.map((label, index) => (<div key={`control-cumple-${index}`} className="grid grid-cols-[44px_minmax(0,1fr)_120px_120px] gap-2 border-t px-2 py-2 text-sm items-start">
                    <div className="text-center">{index + 1}</div>
                    <div className="text-sm leading-snug">{label}</div>
                    <div className="flex justify-center items-center">
                      <Checkbox checked={controlForm.cumple[index]?.cumple} onCheckedChange={(checked) => {
                const value = checked === true;
                setControlForm((prev) => {
                    ...
                    <div className="flex justify-center items-center">
                      <Checkbox checked={controlForm.cumple[index]?.noCumple} onCheckedChange={(checked) => {
                const value = checked === true;
                setControlForm((prev) => {
                    const cumple = [...prev.cumple];
            ...
            <div className="grid gap-2">
              ...
              <Textarea value={controlForm.observaciones} onChange={(e) => setControlForm({ ...controlForm, observaciones: e.target.value })} rows={4}/>
            </div>
          </div>
          <DialogFooter className="pt-4">
            <Button variant="outline" onClick={() => setShowControlModal(false)}>Cancelar</Button>
            <Button disabled={controlLoading} onClick={async () => {
      ...
      <Dialog open={showCartaInvModal} onOpenChange={setShowCartaInvModal}>
        <DialogContent>
          ...
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              <Label>ID Proyecto</Label>
              <Input type="number" value={cartaInvForm.idProyecto} onChange={(e) => setCartaInvForm({ ...cartaInvForm, idProyecto: e.target.value })} placeholder="ID del proyecto"/>
            </div>
            <div className="grid gap-2">
            ...
            <div className="grid gap-2">
              <Label>Fecha</Label>
              <Input type="date" value={cartaInvForm.fecha} onChange={(e) => setCartaInvForm({ ...cartaInvForm, fecha: e.target.value })}/>
            </div>
            <div className="grid gap-2">
              <Label>Destinatario (nombre)</Label>
              <Input value={cartaInvForm.destinatarioNombre} onChange={(e) => setCartaInvForm({ ...cartaInvForm, destinatarioNombre: e.target.value })} placeholder="Nombre del tutor"/>
            </div>
            <div className="grid gap-2">
              <Label>Destinatario (cargo)</Label>
              <Input value={cartaInvForm.destinatarioCargo} onChange={(e) => setCartaInvForm({ ...cartaInvForm, destinatarioCargo: e.target.value })} placeholder="DOCENTE DE LA EMI"/>
            </div>
          ...
          <DialogFooter>
            ...
            <Button disabled={cartaInvLoading} onClick={async () => {
      ...
      <Dialog open={showCartaAceModal} onOpenChange={setShowCartaAceModal}>
        <DialogContent>
          ...
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              <Label>ID Proyecto</Label>
              <Input type="number" value={cartaAceForm.idProyecto} onChange={(e) => setCartaAceForm({ ...cartaAceForm, idProyecto: e.target.value })} placeholder="ID del proyecto"/>
            </div>
            <div className="grid gap-2">
              <Label>CITE (opcional)</Label>
            ...
            <div className="grid gap-2">
              <Label>Fecha</Label>
              <Input type="date" value={cartaAceForm.fecha} onChange={(e) => setCartaAceForm({ ...cartaAceForm, fecha: e.target.value })}/>
            </div>
            <p className="text-xs text-muted-foreground">
          ...
          <DialogFooter>
            ...
            <Button disabled={cartaAceLoading} onClick={async () => {
      ...
      <Dialog open={showActaModal} onOpenChange={setShowActaModal}>
        <DialogContent className="max-w-2xl">
          ...
          <div className="grid gap-3 md:grid-cols-2">
            <div className="grid gap-2">
              <Label>ID Proyecto</Label>
              <Input type="number" value={actaForm.idProyecto} onChange={(e) => setActaForm({ ...actaForm, idProyecto: e.target.value })} placeholder="ID del proyecto"/>
            </div>
            <div className="grid gap-2">
              <Label>CITE</Label>
            ...
            <div className="grid gap-2">
              <Label>Fecha</Label>
              <Input type="date" value={actaForm.fecha} onChange={(e) => setActaForm({ ...actaForm, fecha: e.target.value })}/>
            </div>
            <div className="grid gap-2">
              <Label>Fase</Label>
              <Select value={actaForm.fase} onValueChange={(value) => setActaForm({ ...actaForm, fase: value })} disabled={phasesLoading || phases.length === 0}>
                <SelectTrigger>
                  <SelectValue placeholder={phasesLoading ? "Cargando fases..." : "Selecciona fase"}/>
                </SelectTrigger>
                <SelectContent>
          ...
          <DialogFooter className="pt-4">
            ...
            <Button disabled={actaLoading} onClick={async () => {
      ...
      <Dialog open={showMemoModal} onOpenChange={setShowMemoModal}>
        <DialogContent className="max-w-xl">
          ...
          <div className="grid md:grid-cols-2 gap-3">
            <div className="grid gap-2">
              <Label>ID Proyecto</Label>
              <Input type="number" value={memoForm.idProyecto} onChange={(e) => setMemoForm({ ...memoForm, idProyecto: e.target.value })} placeholder="ID del proyecto"/>
            </div>
            <div className="grid gap-2">
              <Label>CITE</Label>
              <Input value={memoForm.cite} onChange={(e) => setMemoForm({ ...memoForm, cite: e.target.value })} placeholder="Ej. MEMO-123/2024"/>
            </div>
            <div className="grid gap-2">
              <Label>Ciudad</Label>
              <Input value={memoForm.ciudad} onChange={(e) => setMemoForm({ ...memoForm, ciudad: e.target.value })} placeholder="Ej. LA PAZ"/>
            </div>
            <div className="grid gap-2">
              <Label>Fecha</Label>
              <Input type="date" value={memoForm.fecha} onChange={(e) => setMemoForm({ ...memoForm, fecha: e.target.value })}/>
            </div>
            <div className="grid gap-2">
              <Label>Fecha de defensa</Label>
              <Input type="date" value={memoForm.fechaDefensa} onChange={(e) => setMemoForm({ ...memoForm, fechaDefensa: e.target.value })}/>
            </div>
            <div className="grid gap-2">
              <Label>Hora de defensa</Label>
              <Input type="time" value={memoForm.horaDefensa} onChange={(e) => setMemoForm({ ...memoForm, horaDefensa: e.target.value })}/>
            </div>
            <div className="col-span-full text-xs text-muted-foreground">
          ...
          <DialogFooter className="pt-4">
            ...
            <Button disabled={memoLoading} onClick={async () => {
      ...
      <Dialog open={showMemoAsignacionModal} onOpenChange={setShowMemoAsignacionModal}>
        <DialogContent className="max-w-xl">
          ...
          <div className="grid md:grid-cols-2 gap-3">
            <div className="grid gap-2">
              <Label>ID Proyecto</Label>
              <Input type="number" value={memoAsignacionForm.idProyecto} onChange={(e) => setMemoAsignacionForm({ ...memoAsignacionForm, idProyecto: e.target.value })} placeholder="ID del proyecto"/>
            </div>
            <div className="grid gap-2">
              <Label>CITE</Label>
              <Input value={memoAsignacionForm.cite} onChange={(e) => setMemoAsignacionForm({ ...memoAsignacionForm, cite: e.target.value })} placeholder="Ej. MEMO-123/2024"/>
            </div>
            <div className="grid gap-2">
              <Label>Ciudad</Label>
              <Input value={memoAsignacionForm.ciudad} onChange={(e) => setMemoAsignacionForm({ ...memoAsignacionForm, ciudad: e.target.value })} placeholder="Ej. LA PAZ"/>
            </div>
            <div className="grid gap-2">
              <Label>Fecha</Label>
              <Input type="date" value={memoAsignacionForm.fecha} onChange={(e) => setMemoAsignacionForm({ ...memoAsignacionForm, fecha: e.target.value })}/>
            </div>
            <div className="col-span-full text-xs text-muted-foreground">
          ...
          <DialogFooter className="pt-4">
            ...
            <Button disabled={memoAsignacionLoading} onClick={async () => {
      ...
      <Dialog open={showAvalModal} onOpenChange={setShowAvalModal}>
        <DialogContent className="max-w-xl">
          ...
          <div className="grid md:grid-cols-2 gap-3">
            <div className="grid gap-2">
              <Label>ID Proyecto</Label>
              <Input type="number" value={avalForm.idProyecto} onChange={(e) => setAvalForm({ ...avalForm, idProyecto: e.target.value })} placeholder="ID del proyecto"/>
            </div>
            <div className="grid gap-2">
              <Label>Fecha</Label>
              <Input type="date" value={avalForm.fecha} onChange={(e) => setAvalForm({ ...avalForm, fecha: e.target.value })}/>
            </div>
            <div className="grid gap-2">
              <Label>Fase</Label>
              <Select value={avalForm.fase} onValueChange={(value) => setAvalForm({ ...avalForm, fase: value })} disabled={phasesLoading || phases.length === 0}>
                <SelectTrigger>
                  <SelectValue placeholder={phasesLoading ? "Cargando fases..." : "Selecciona fase"}/>
                </SelectTrigger>
                <SelectContent>
            ...
            <div className="grid gap-2">
              ...
              <Select value={avalForm.firmante} onValueChange={(value) => setAvalForm({ ...avalForm, firmante: value as "tg" | "tutor" | "rev1" | "rev2" })}>
                <SelectTrigger>
          ...
          <DialogFooter className="pt-4">
            ...
            <Button disabled={avalLoading} onClick={async () => {
      ...
      <Dialog open={showPreview} onOpenChange={handlePreviewOpenChange}>
        <DialogContent className="max-w-5xl">
          ...
          {previewUrl ? (<iframe title={previewName || "Documento"} src={previewUrl} className="w-full h-[70vh] border rounded-md"/>) : (<div className="flex items-center justify-center h-[50vh]">
              <Loader2 className="w-8 h-8 animate-spin text-primary"/>
            </div>)}
          <DialogFooter className="pt-4">
