...
export function ProfileImage({ user, alt = "Perfil", className, containerClassName, overlayClassName = "rounded-lg", fallbackSrc = "/placeholder.svg?height=200&width=200&query=profile", showLoader = false, children, }: ProfileImageProps) {
    const { src, isLoading } = useProfileImage(user);
    return (<div className={cn("relative inline-block", containerClassName)}>
      <img src={src || fallbackSrc} alt={alt} className={className}/>
      {showLoader && isLoading && (<div className={cn("absolute inset-0 bg-background/70 flex items-center justify-center", overlayClassName)}>
          <Spinner />
        </div>)}
      {children}
    </div>);
