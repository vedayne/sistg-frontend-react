...
type SpecificSeatType = 'La Paz-Irpavi' | 'La Paz-Central' | 'La Paz-Alto Irpavi' | 'Cochabamba-Lanza' | 'Cochabamba-Muyurina' | 'Santa Cruz' | 'Riberalta' | 'Tropico';
...
const getDefaultSpecificSeat = (seat: SeatType): SpecificSeatType => {
    if (seat === 'La Paz')
        return 'La Paz-Central';
    if (seat === 'Cochabamba')
        return 'Cochabamba-Lanza';
...
const SeatButton: React.FC<SeatButtonProps> = ({ text, seat, isSelected, onClick }) => {
    ...
    return (<button onClick={onClick} className={`${baseClasses} ${isSelected ? selectedClasses : unselectedClasses}`} aria-pressed={isSelected}>
      {text}
    </button>);
};
...
const MapIframe: React.FC<MapIframeProps> = ({ embedUrl }) => {
    return (<iframe src={embedUrl} allowFullScreen style={{ border: 0, width: '100%', height: '350px' }} title="Mapa EMI" loading="lazy"/>);
};
...
const InfoCard: React.FC<InfoCardProps> = ({ seatInfo, isVisible }) => {
    if (!isVisible)
        return null;
    return (<div className="glass neon-border rounded-lg p-6 animate-fade-in">
      ...
      <div className="space-y-3 text-sm font-semibold text-slate-700 dark:text-white">
        ...
        {seatInfo.phones.length > 0 && (<p>
            📞 {seatInfo.phones.join(' | ')}
          </p>)}
        
        <p>
          📱 {seatInfo.cellphones.map((cellphone, index) => (<React.Fragment key={cellphone}>
              <a href={`https://wa.me/${formatPhoneForWhatsApp(cellphone)}`} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:text-blue-800 hover:underline">
                {cellphone}
              </a>
        ...
        <p>
          <a href={`mailto:${seatInfo.email}`} className="text-blue-600 hover:text-blue-800 hover:underline">
            📧 {seatInfo.email}
          </a>
        </p>
        
        <p>
          <a href={`https://www.${seatInfo.website}`} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:text-blue-800 hover:underline">
            🌐 {seatInfo.website}
          </a>
        </p>
...
const InteractiveMapSection: React.FC = () => {
    ...
    const specificSeatOptions = useMemo((): SpecificSeatType[] => {
        if (selectedSeat === 'La Paz')
            return LA_PAZ_OPTIONS;
        if (selectedSeat === 'Cochabamba')
            return COCHABAMBA_OPTIONS;
        return [];
    }, [selectedSeat]);
    ...
    return (<section className="py-24 px-4 interactive-map-section">
      <style>{`
        * {
      ...
      <div className="w-full">
        ...
        <div className="flex gap-4 justify-center my-8 flex-wrap">
          {SEATS_DATA.map((seatData) => (<SeatButton key={seatData.seat} text={seatData.title} seat={seatData.seat as SeatType} isSelected={selectedSeat === seatData.seat} onClick={() => handleSeatSelect(seatData.seat as SeatType)}/>))}
        </div>

        
        {showSpecificButtons && (<div className="flex justify-center w-full mx-auto gap-4 my-5 flex-wrap">
            {specificSeatOptions.map((option) => (<SpecificSeatButton key={option} option={option} isSelected={specificSeat === option} onClick={() => handleSpecificSeatSelect(option)}/>))}
          </div>)}
        ...
        <div className="mt-12 max-w-3xl mx-auto">
          {currentSeatInfo && (<InfoCard seatInfo={currentSeatInfo} isVisible={true}/>)}
        </div>
      </div>
    </section>);
};
