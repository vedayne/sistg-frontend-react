...
export default function Sidebar({ currentPage, onPageChange }: SidebarProps) {
    ...
    return (<aside className={`${isCollapsed ? 'w-20' : 'w-64'} bg-primary text-primary-foreground flex flex-col overflow-hidden border-r dark:border-slate-800 transition-all duration-300`}>
      ...
      <div className="p-6 border-b border-primary/20">
        <div className="flex items-center justify-between gap-3">
          ...
          <button onClick={() => setIsCollapsed(!isCollapsed)} className="p-2 hover:bg-primary/40 rounded-lg transition-colors flex-shrink-0" title={isCollapsed ? "Expandir sidebar" : "Colapsar sidebar"}>
      ...
      <nav className="flex-1 p-4 space-y-2 overflow-y-auto scrollbar-thin scrollbar-thumb-secondary scrollbar-track-primary/20">
        {menuItems.map((item) => {
            ...
            return (<button key={item.id} onClick={() => onPageChange(item.id)} className={`w-full text-left rounded-lg transition-all font-medium flex items-center ${isCollapsed ? 'px-3 py-3 justify-center' : 'px-4 py-3 gap-3'} ${currentPage === item.id
                    ? "bg-secondary text-secondary-foreground shadow-md"
                    : "hover:bg-primary/80 text-primary-foreground/90 hover:text-primary-foreground"}`} title={isCollapsed ? item.label : undefined}>
        ...
        <Popover>
          <PopoverTrigger asChild>
            <button className={`w-full flex items-center hover:bg-primary/40 p-2 rounded-md transition-colors group ${isCollapsed ? 'justify-center' : 'gap-3 text-left'}`}>
              <ProfileImage user={user} className="w-8 h-8 rounded-full bg-secondary/20 flex items-center justify-center text-secondary-foreground shrink-0" containerClassName="relative" overlayClassName="rounded-full"/>
              {!isCollapsed && (<>
          ...
          <PopoverContent className="w-64 p-3 mb-2" side="top" align={isCollapsed ? "start" : "center"}>
            ...
            <div className="space-y-2">
              {rolesList.map((role, idx) => (<div key={idx} className="bg-muted/50 p-2 rounded text-xs border border-muted-foreground/10">
